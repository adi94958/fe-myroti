...
const FormUpdatePemilik = () => {
  ...
  useEffect(() => {
    const dataPemilik = JSON.parse(localStorage.getItem('lsDataPemilik'))
    setCount({
      nama: dataPemilik.nama.length,
      username: dataPemilik.username.length,
      password: dataPemilik.password.length,
    })
    setFormData({
      id: dataPemilik.id,
      nama: dataPemilik.nama,
      username: dataPemilik.username,
      password: dataPemilik.password,
    })
  }, [])
    ...
    <>
      <CCard>
        <CForm onSubmit={handleSubmit}>
          <CCardHeader>Form Edit Akun Pemilik</CCardHeader>
          <CCardBody>
            <CRow>
              <CCol xs={12}>
...
export default FormUpdatePemilik
